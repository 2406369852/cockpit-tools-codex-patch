...
pub async fn confirm_codex_batch_import(
    ...
    let result =
        codex_account::confirm_codex_batch_import(&app, &session_id, &item_ids, actual_spend_cny)?;
    reactivate_imported_current_if_needed(&result.imported).await;
    let _ = crate::modules::tray::update_tray_menu(&app);
    Ok(result)
}
...
#[tauri::command]
pub async fn update_codex_account_actual_spend(
    account_id: String,
    actual_spend_cny: Option<f64>,
) -> Result<CodexAccount, String> {
    codex_account::update_account_actual_spend(&account_id, actual_spend_cny)
}
...
#[tauri::command]
pub async fn codex_local_access_restore_after_restart() -> Result<CodexLocalAccessState, String> {
    codex_local_access::restore_local_access_gateway_after_restart().await
}
