...
fn read_account_actual_spend_cny(value: &serde_json::Value) -> Option<f64> {
    let raw = value
        .get("actual_spend_cny")
        .or_else(|| value.get("actualSpendCny"))?;
    let parsed = raw.as_f64().or_else(|| {
        raw.as_str()
            .and_then(|item| item.trim().parse::<f64>().ok())
    })?;
    normalize_actual_spend_cny(Some(parsed)).ok().flatten()
}

fn apply_account_actual_spend_from_value(
    account: &mut CodexAccount,
    value: &serde_json::Value,
) -> bool {
    let Some(actual_spend_cny) = read_account_actual_spend_cny(value) else {
        return false;
    };
    account.actual_spend_cny = Some(actual_spend_cny);
    true
}

fn save_account_actual_spend_from_value_if_present(
    account: &mut CodexAccount,
    value: &serde_json::Value,
) -> Result<(), String> {
    if apply_account_actual_spend_from_value(account, value) {
        save_account(account)?;
    }
    Ok(())
}
...
fn apply_compat_account_metadata(
    ...
    account.actual_spend_cny = read_account_actual_spend_cny(value).or(account.actual_spend_cny);
...
fn apply_api_key_import_metadata(account: &mut CodexAccount, value: &serde_json::Value) {
    ...
    apply_account_actual_spend_from_value(account, value);
    apply_account_sensitive_note_metadata(account, value);
...
fn load_account_with_summary(
    ...
    {
        let restored_billing =
            crate::modules::codex_billing::reconcile_loaded_account(&mut account);
    ...
    let _restored_billing = crate::modules::codex_billing::reconcile_loaded_account(&mut account);
...
pub fn save_account(account: &CodexAccount) -> Result<(), String> {
    let account = crate::modules::codex_billing::account_for_save(account);
...
fn import_account_struct(account: CodexAccount) -> Result<CodexAccount, String> {
    if let Some(identity) = account.agent_identity.clone() {
        ...
        if let Some(actual_spend_cny) = account.actual_spend_cny {
            imported.actual_spend_cny = normalize_actual_spend_cny(Some(actual_spend_cny))?;
            changed = true;
        }
        if changed {
    ...
    if account.is_api_key_auth() || account.openai_api_key.is_some() {
        ...
        if let Some(actual_spend_cny) = account.actual_spend_cny {
            api_acc.actual_spend_cny = normalize_actual_spend_cny(Some(actual_spend_cny))?;
            changed = true;
        }
        if changed {
...
fn pending_oauth_account_from_value(value: &serde_json::Value) -> Option<CodexAccount> {
    ...
    apply_account_actual_spend_from_value(&mut account, value);
...
fn merge_access_token_import_hints(
    ...
    if primary.actual_spend_cny.is_none() {
        primary.actual_spend_cny = fallback.actual_spend_cny;
    }
    if primary.two_factor_secret.is_none() {
        primary.two_factor_secret = fallback.two_factor_secret;
    }
...
fn extract_access_token_import_hints_from_value(
    ...
    CodexAccessTokenImportHints {
        ...
        actual_spend_cny: read_account_actual_spend_cny(value),
...
fn extract_codex_session_candidate_from_value(
    ...
    if let Some(id_token) = first_json_string(&session, &[&["idToken"], &["id_token"]]) {
        ...
        return Some(CodexJsonImportCandidate::FullToken {
            ...
            note_update,
            actual_spend_cny: session_hints.actual_spend_cny,
        });
    }
...
async fn upsert_account_from_refresh_token(
    ...
    if let Some(actual_spend_cny) = normalize_actual_spend_cny(actual_spend_cny)? {
        account.actual_spend_cny = Some(actual_spend_cny);
        save_account(&account)?;
    }
    Ok(account)
...
fn upsert_account_from_access_token_with_hints(
    ...
    let actual_spend_cny = normalize_actual_spend_cny(hints.actual_spend_cny)?;
    let access_token =
    ...
    if let Some(actual_spend_cny) = actual_spend_cny {
        account.actual_spend_cny = Some(actual_spend_cny);
    }
...
async fn import_codex_candidate(
    ...
    match candidate {
        ...
        CodexJsonImportCandidate::RefreshToken {
            ...
            note_update,
            actual_spend_cny,
        } => upsert_account_from_refresh_token(refresh_token, note_update, actual_spend_cny).await,
    }
}
    ...
    for (index, line) in lines.into_iter().enumerate() {
        ...
        for value in values {
            if let Some(identity) = parse_agent_identity_from_value(&value)? {
                let mut account = upsert_agent_identity_account(identity)?;
                save_account_actual_spend_from_value_if_present(&mut account, &value)?;
...
async fn import_sub2api_export_from_value(
    ...
    for (index, item) in accounts.iter().enumerate() {
        ...
        if let Some(identity) = parse_agent_identity_from_value(item)? {
            let mut account = upsert_agent_identity_account(identity)?;
            save_account_actual_spend_from_value_if_present(&mut account, item)?;
...
fn preview_account_from_full_tokens(
    ...
    apply_account_note_update_if_present(&mut account, note_update);
    account.actual_spend_cny = normalize_actual_spend_cny(actual_spend_cny)?;
    Ok(account)
}
...
fn preview_account_from_access_token(
    ...
    account.actual_spend_cny = normalize_actual_spend_cny(hints.actual_spend_cny)?;
...
async fn codex_batch_import_draft_from_value(
    ...
    if let Some(identity) = parse_agent_identity_from_value(&value)? {
        let mut account = build_agent_identity_account_draft(identity)?;
        apply_account_actual_spend_from_value(&mut account, &value);
...
fn selected_codex_batch_import_items(
    items: &[CodexBatchImportCachedItem],
    item_ids: &[String],
) -> Vec<CodexBatchImportCachedItem> {
    let selected: HashSet<&str> = item_ids.iter().map(String::as_str).collect();
    items
        .iter()
        .filter(|cached| selected.contains(cached.preview.item_id.as_str()))
        .cloned()
        .collect()
}
...
pub fn confirm_codex_batch_import(
    ...
    ensure_storage_writable_for_import()?;
    let actual_spend_cny = normalize_actual_spend_cny(actual_spend_cny)?;
    ensure_codex_batch_import_session_loaded(session_id)?;
    let (cached_items, cancel, session_snapshot) = {
        ...
        (
            selected_codex_batch_import_items(&session.items, item_ids),
            session.cancel.clone(),
            session.clone(),
        )
    };
        ...
        let result = (|| -> Result<CodexAccount, String> {
            let mut account = match draft {
                ...
                CodexBatchImportDraft::FullToken {
                    ...
                    subscription_active_until_hint,
                    note_update,
                    actual_spend_cny: imported_actual_spend_cny,
                } => {
                    let mut account = upsert_account_with_import_hints(
                        tokens,
                        account_id_hint,
                    ...
                    save_account_note_update_if_present(&mut account, note_update)?;
                    if let Some(imported_actual_spend_cny) =
                        normalize_actual_spend_cny(imported_actual_spend_cny)?
                    {
                        account.actual_spend_cny = Some(imported_actual_spend_cny);
                        save_account(&account)?;
                    }
                    account
                }
...
mod tests {
    ...
    #[test]
    fn batch_import_price_selection_excludes_unselected_accounts() {
        let build_cached = |item_id: &str, price: f64| {
            let mut account = CodexAccount::new(
                format!("account-{item_id}"),
                format!("{item_id}@example.com"),
                CodexTokens {
                    id_token: String::new(),
                    access_token: format!("at-{item_id}"),
                    refresh_token: None,
                },
            );
            account.actual_spend_cny = Some(price);
            super::CodexBatchImportCachedItem {
                preview: super::CodexBatchImportItem {
                    item_id: item_id.to_string(),
                    source: format!("{item_id}.json"),
                    label: item_id.to_string(),
                    account_id: Some(account.id.clone()),
                    email: Some(account.email.clone()),
                    account_type: "OAuth".to_string(),
                    provider: None,
                    quota_status: "unchecked".to_string(),
                    quota_error: None,
                ...
                draft: Some(super::CodexBatchImportDraft::Account(account)),
                quota: None,
            }
        };
        let items = vec![
            build_cached("selected", 10.0),
            build_cached("skipped", 20.0),
        ];
        let selected_ids = vec!["selected".to_string()];

        let selected = super::selected_codex_batch_import_items(&items, &selected_ids);

        assert_eq!(selected.len(), 1);
        assert_eq!(selected[0].preview.item_id, "selected");
        match items[1].draft.as_ref().expect("unselected draft") {
            super::CodexBatchImportDraft::Account(account) => {
                assert_eq!(account.actual_spend_cny, Some(20.0));
            }
            _ => panic!("expected account draft"),
        }
    }
...
{"id_token":"id-2","access_token":"access-2"}"#;
    ...
    #[test]
    fn import_full_cockpit_account_json_preserves_actual_spend() {
        let _lock = crate::modules::test_support::env_lock()
            .lock()
            .unwrap_or_else(|err| err.into_inner());
        let _env = TestEnvGuard::new("codex-full-account-price-import-test");
        let mut source = CodexAccount::new(
            "exported-account-id".to_string(),
            "priced-import@example.com".to_string(),
            make_codex_tokens(
                "priced-import@example.com",
                "priced-import-account",
                "priced-import-org",
                "priced-import",
                "rt-priced-import",
            ),
        );
        source.account_note = Some("keep imported note".to_string());
        source.actual_spend_cny = Some(48.888);
        let payload = serde_json::to_string(&source).expect("serialize full account");
        let runtime = tokio::runtime::Runtime::new().expect("create runtime");

        let imported = runtime
            .block_on(import_from_json(&payload))
            .expect("import full Cockpit account");

        assert_eq!(imported.len(), 1);
        assert_eq!(imported[0].actual_spend_cny, Some(48.89));
        assert_eq!(
            imported[0].account_note.as_deref(),
            Some("keep imported note")
        );
        let persisted = load_account(&imported[0].id).expect("load imported account");
        assert_eq!(persisted.actual_spend_cny, Some(48.89));
        assert_eq!(
            persisted.account_note.as_deref(),
            Some("keep imported note")
        );
    }
    ...
    #[test]
    fn update_and_clear_actual_spend_preserves_account_note() {
        let _lock = crate::modules::test_support::env_lock()
            .lock()
            .unwrap_or_else(|err| err.into_inner());
        let _env = TestEnvGuard::new("codex-actual-spend-update-test");
        let mut account = CodexAccount::new_api_key(
            "priced-account".to_string(),
            "priced@example.com".to_string(),
            "sk-priced-test".to_string(),
            CodexApiProviderMode::Custom,
        ...
        account.account_note = Some("keep this note".to_string());
        save_account(&account).expect("save priced account");

        let priced = super::update_account_actual_spend(&account.id, Some(35.678))
            .expect("set actual spend");
        assert_eq!(priced.actual_spend_cny, Some(35.68));
        assert_eq!(priced.account_note.as_deref(), Some("keep this note"));

        let cleared =
            super::update_account_actual_spend(&account.id, None).expect("clear actual spend");
        assert_eq!(cleared.actual_spend_cny, None);
        assert_eq!(cleared.account_note.as_deref(), Some("keep this note"));

        let persisted = load_account(&account.id).expect("load cleared account");
        assert_eq!(persisted.actual_spend_cny, None);
        assert_eq!(persisted.account_note.as_deref(), Some("keep this note"));
    }
        ...
        let content = r#"[
  {
    ...
    "mail_url": "https://mail.example.test/inbox?mail=dddd"
        ...
        assert_eq!(account.actual_spend_cny, Some(35.68));
        ...
        assert_eq!(persisted.actual_spend_cny, Some(35.68));
        assert_eq!(
...
fn normalize_actual_spend_cny(value: Option<f64>) -> Result<Option<f64>, String> {
    crate::modules::codex_billing::normalize_actual_spend_cny(value)
}

pub fn update_account_actual_spend(
    account_id: &str,
    actual_spend_cny: Option<f64>,
) -> Result<CodexAccount, String> {
    let mut account =
        load_account(account_id).ok_or_else(|| format!("账号不存在: {}", account_id))?;
    account.actual_spend_cny =
        crate::modules::codex_billing::set_actual_spend(account_id, actual_spend_cny)?;
    save_account(&account)?;
    Ok(account)
}
