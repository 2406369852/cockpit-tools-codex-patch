...
import {
  prepareCodexLocalAccessForRestart,
  restoreCodexLocalAccessAfterRestartFailure,
...
function MainApp() {
  ...
  const prepareCodexLocalAccessBeforeRelaunch = useCallback(async () => {
    setUpdateRetryStatus(
      t('update_notification.preparingApiServiceHandoff', '正在准备 API 服务热更新交接...'),
    );
    try {
      const state = await prepareCodexLocalAccessForRestart();
      writeUpdateLog(
        'info',
        `应用重启前已准备 Codex API 服务热更新交接: enabled=${Boolean(state.collection?.enabled)}, running=${state.running}`,
      );
    } catch (error) {
      const compactError = sanitizeUpdaterErrorMessage(error);
      writeUpdateLog(
        'warn',
        `应用重启前准备 Codex API 服务热更新交接失败，将继续重启: error=${compactError}`,
      );
      setUpdateRetryStatus(
        t(
          'update_notification.prepareApiServiceHandoffFailedContinue',
          'API 服务热更新交接未就绪，将继续重启并自动恢复...',
        ),
      );
    }
  }, [t, writeUpdateLog]);
  ...
  const restoreCodexLocalAccessAfterRelaunchFailure = useCallback(async () => {
    await restoreCodexLocalAccessAfterRestartFailure().catch((error) => {
      writeUpdateLog(
        'warn',
  ...
  const handleApplyPendingUpdate = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = failureStage === 'prepare'
        ? t('update_notification.prepareApiServiceHandoffFailed', '无法准备 API 服务热更新交接，请稍后重试。')
        : failureStage === 'install'
