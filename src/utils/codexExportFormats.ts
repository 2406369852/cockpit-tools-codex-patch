...
function appendActualSpendCny(target: JsonRecord, account: CodexAccount): void {
  const value = account.actual_spend_cny;
  if (
    typeof value === 'number' &&
    ...
    target.actual_spend_cny = Math.round(value * 100) / 100;
  }
}
...
function toCockpitToolsPortableStorage(
  ...
  const payload = hasAgentIdentity(account)
  ...
  appendActualSpendCny(payload, account);
  return payload;
}
