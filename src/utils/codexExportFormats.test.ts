...
test('Cockpit Tools export preserves the local actual spend price', () => {
  const account: CodexAccount = {
    id: 'codex-priced-fixture',
  ...
  const cockpitExport = JSON.parse(
    transformCodexExportJson(JSON.stringify([account]), 'cockpit_tools'),
  ) as Array<Record<string, unknown>>;
  const cpaExport = JSON.parse(
    transformCodexExportJson(JSON.stringify([account]), 'cpa'),
  ) as Record<string, unknown>;

  assert.equal(cockpitExport[0].actual_spend_cny, 35.68);
  assert.equal(cpaExport.actual_spend_cny, undefined);
});
