...
export async function confirmCodexBatchImport(
  sessionId: string,
  itemIds: string[],
  actualSpendCny?: number | null,
): Promise<CodexBatchImportConfirmResult> {
  return await invoke('confirm_codex_batch_import', {
    sessionId,
    itemIds,
    actualSpendCny: actualSpendCny ?? null,
  });
}
...
export async function updateCodexAccountActualSpend(
  accountId: string,
  actualSpendCny: number | null,
): Promise<CodexAccount> {
  return await invoke('update_codex_account_actual_spend', {
    accountId,
    actualSpendCny,
  });
}
