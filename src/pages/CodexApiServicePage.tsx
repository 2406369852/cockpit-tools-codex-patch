...
import {
  ...
  formatCodexActualSpendCny,
  formatCodexActualSpendCnyDraft,
  parseCodexActualSpendCnyInput,
} from "../utils/codexAccountCost";
import {
...
export function CodexApiServicePage() {
  ...
  const [accountCostEditorOpen, setAccountCostEditorOpen] = useState(false);
  const [accountCostDrafts, setAccountCostDrafts] = useState<
    Record<string, string>
  >({});
  const [accountCostBulkInput, setAccountCostBulkInput] = useState("");
  const [accountCostError, setAccountCostError] = useState("");
  ...
  const resetAccountCostDrafts = () => {
    setAccountCostDrafts(
      Object.fromEntries(
        memberAccounts.map((account) => [
          account.id,
          formatCodexActualSpendCnyDraft(account.actual_spend_cny),
        ]),
      ),
    );
    setAccountCostBulkInput("");
    setAccountCostError("");
  };

  const handleOpenAccountCostEditor = () => {
    resetAccountCostDrafts();
    setAccountCostEditorOpen(true);
  };

  const handleCloseAccountCostEditor = () => {
    setAccountCostEditorOpen(false);
    setAccountCostError("");
  };

  const handleApplyAccountCostBulk = () => {
    const actualSpendCny = parseCodexActualSpendCnyInput(accountCostBulkInput);
    if (actualSpendCny === undefined || actualSpendCny === null) {
      setAccountCostError(
        t(
          "codex.apiService.accountCost.invalid",
          "价格必须是 0 到 1000000 之间、最多两位小数的数字",
        ),
      );
      return;
    }
    const value = formatCodexActualSpendCnyDraft(actualSpendCny);
    setAccountCostDrafts(
      Object.fromEntries(memberAccounts.map((account) => [account.id, value])),
    );
    setAccountCostError("");
  };

  const handleSaveAccountCosts = async () => {
    const updates: Array<{ accountId: string; actualSpendCny: number | null }> = [];
    for (const account of memberAccounts) {
      const actualSpendCny = parseCodexActualSpendCnyInput(
        accountCostDrafts[account.id] ?? "",
      );
      if (actualSpendCny === undefined) {
        setAccountCostError(
          t(
            "codex.apiService.accountCost.invalid",
            "价格必须是 0 到 1000000 之间、最多两位小数的数字",
      ...
      updates.push({ accountId: account.id, actualSpendCny });
    }
    setBusy(true);
    setAccountCostError("");
    try {
      const results = await Promise.allSettled(
        updates.map(({ accountId, actualSpendCny }) =>
          updateCodexAccountActualSpend(accountId, actualSpendCny),
        ),
      );
      ...
      if (failed.length > 0) {
        setAccountCostError(
          t("codex.apiService.accountCost.partialFailure", {
            success: updates.length - failed.length,
            failed: failed.length,
            defaultValue: "已保存 {{success}} 个账号，{{failed}} 个账号保存失败",
          }),
        );
        return;
      }
      ...
      setNotice(
        t(
          "codex.apiService.accountCost.saved",
          "账号价格已保存",
        ),
      );
    } finally {
  ...
  return (
    <div className="codex-api-service-page">
              ...
              <div className="codex-api-service-panel-head">
                ...
                <div className="codex-api-service-head-actions">
                  ...
                  <button
                    type="button"
                    className="btn btn-secondary btn-sm"
                    onClick={handleOpenAccountCostEditor}
                    disabled={busy || memberAccounts.length === 0}
                  >
                    <BadgeDollarSign size={14} />
                    {t("codex.apiService.accountCost.action", "批量填价格")}
                  </button>
                  <button
                    type="button"
                    className="btn btn-secondary btn-sm"
              ...
              <div className="codex-api-service-account-grid">
                {memberAccounts.length === 0 ? (
                  ...
                  memberAccounts.map((account) => {
                    ...
                    const accountCostMultiplier =
                      calculateCodexAccountCostMultiplier(
                        account.actual_spend_cny,
                        stat?.usage.estimatedCostUsd,
                      );
                    return (
                      <div
                        key={account.id}
                      ...
                      >
                        ...
                        <div className="codex-api-service-account-meta">
                          ...
                          <span className="codex-api-service-account-meta-cost">
                            {t("codex.apiService.accountCost.actualSpend", {
                              value: formatCodexActualSpendCny(
                                account.actual_spend_cny,
                              ),
                              defaultValue: "实际 {{value}}",
                            })}
                          </span>
                          <span className="codex-api-service-account-meta-multiplier">
                            {t("codex.apiService.accountCost.multiplier", {
                              value: formatCodexAccountCostMultiplier(
                                accountCostMultiplier,
                              ),
                              defaultValue: "倍率 {{value}}",
                            })}
                          </span>
                          <span>
      ...
      {accountCostEditorOpen && (
        <div
          className="modal-overlay codex-api-service-pricing-overlay"
          role="presentation"
        >
          <div
            className="modal codex-api-service-pricing-modal codex-api-service-cost-modal"
            role="dialog"
            aria-modal="true"
            aria-labelledby="codex-api-service-account-cost-title"
          >
            <div className="modal-header">
              <div>
                <h2 id="codex-api-service-account-cost-title">
                  {t("codex.apiService.accountCost.title", "批量填写账号价格")}
                </h2>
                <p className="codex-api-service-pricing-desc">
                  {t(
                    "codex.apiService.accountCost.desc",
                    "填写账号实际价格（人民币），并用本地 API 服务的美元估值计算倍率。价格独立保存，不占用账号备注。",
                  )}
                </p>
              </div>
              <button
                type="button"
                className="modal-close"
                onClick={handleCloseAccountCostEditor}
                aria-label={t("common.close", "关闭")}
              >
                <X size={18} />
              </button>
            ...
            <div className="modal-body codex-api-service-pricing-body">
              {accountCostError && (
                <div className="codex-api-service-message error">
                  <CircleAlert size={15} />
                  <span>{accountCostError}</span>
                </div>
              )}
              <div className="codex-api-service-cost-bulk">
                <label>
                  <span>
                    {t("codex.apiService.accountCost.bulkLabel", "统一价格（元）")}
                  </span>
                  <input
                    type="text"
                    inputMode="decimal"
                    value={accountCostBulkInput}
                    onChange={(event) =>
                      setAccountCostBulkInput(event.target.value)
                    }
                    placeholder={t(
                      "codex.apiService.accountCost.placeholder",
                      "例如 35.00",
                    )}
                    disabled={busy}
                  />
                ...
                <button
                  type="button"
                  className="btn btn-secondary btn-sm"
                  onClick={handleApplyAccountCostBulk}
                  disabled={busy || memberAccounts.length === 0}
                >
                  {t("codex.apiService.accountCost.applyAll", "应用到全部")}
                </button>
              </div>
              <div className="codex-api-service-cost-table">
                <div className="codex-api-service-cost-row is-head">
                  <span>{t("codex.apiService.accountCost.account", "账号")}</span>
                  <span>{t("codex.apiService.accountCost.priceCny", "价格（CNY）")}</span>
                  <span>{t("codex.apiService.accountCost.usedUsd", "已用估值（USD）")}</span>
                  <span>{t("codex.apiService.accountCost.ratio", "倍率")}</span>
                </div>
                {memberAccounts.map((account) => {
                  ...
                  const parsed = parseCodexActualSpendCnyInput(
                    accountCostDrafts[account.id] ?? "",
                  );
                  const multiplier = calculateCodexAccountCostMultiplier(
                    typeof parsed === "number" ? parsed : null,
                    stat?.usage.estimatedCostUsd,
                  );
                  return (
                    <div
                      key={account.id}
                      className="codex-api-service-cost-row"
                    >
                      <div className="codex-api-service-cost-account">
                        <strong title={presentation.displayName}>
                          {maskAccountText(presentation.displayName)}
                        </strong>
                        <small>{presentation.planLabel}</small>
                      ...
                      <input
                        type="text"
                        inputMode="decimal"
                        value={accountCostDrafts[account.id] ?? ""}
                        onChange={(event) =>
                          setAccountCostDrafts((current) => ({
                            ...current,
                      ...
                      <strong>
                        {formatCodexAccountCostMultiplier(multiplier)}
                      </strong>
                    </div>
                  );
                })}
            ...
            <div className="modal-footer">
              ...
              <button
                type="button"
                className="btn btn-primary"
                onClick={() => void handleSaveAccountCosts()}
                disabled={busy || memberAccounts.length === 0}
              >
                <Check size={15} />
                {t("common.save", "保存")}
