...
export function CodexAccountsPage() {
  ...
  const [batchImportActualSpendCnyInput, setBatchImportActualSpendCnyInput] =
    useState("");
  ...
  const resetBatchImportState = useCallback(() => {
    ...
    setBatchImportActualSpendCnyInput("");
  ...
  const localAccessUsageByAccountId = useMemo(
    () =>
      new Map(
        (localAccessState?.stats.accounts ?? []).map((item) => [
          item.accountId,
          item.usage,
        ]),
      ),
    [localAccessState?.stats.accounts],
  );
  ...
  const handleConfirmBatchImport = async (
    ...
    const actualSpendCny = parseCodexActualSpendCnyInput(
      batchImportActualSpendCnyInput,
    );
    if (actualSpendCny === undefined) {
      setBatchImportError(
        t(
          "codex.batchImport.bulkPriceInvalid",
  ...
  const formatAccountActualPrice = (account: CodexAccount) =>
    typeof account.actual_spend_cny === "number"
      ? formatCodexActualSpendCny(account.actual_spend_cny)
      : t("codex.accountCost.unset", "未填写");
    ...
    items.map((account) => {
      ...
      return (
        ...
        >
          ...
          <div className="codex-compact-account-summary">
            <span
            ...
            <span
              className="codex-compact-account-price"
              title={`${t("codex.accountCost.actualPrice", "实际价格")} ${formatAccountActualPrice(account)}`}
            >
              <span>{t("codex.accountCost.actualPrice", "实际价格")}</span>
              <strong>{formatAccountActualPrice(account)}</strong>
            </span>
          </div>
  ...
  const renderGridCards = (items: typeof filteredAccounts, groupKey?: string) =>
    items.map((account) => {
      ...
      const accountLocalUsage = localAccessUsageByAccountId.get(account.id);
      const accountEstimatedCostUsd =
        accountLocalUsage?.estimatedCostUsd;
      const accountCostMultiplier = calculateCodexAccountCostMultiplier(
        account.actual_spend_cny,
        accountEstimatedCostUsd,
      );
      return (
        <div
          key={groupKey ? `${groupKey}-${account.id}` : account.id}
        ...
        >
          ...
          <div className="codex-account-cost-strip">
            <div
              title={t(
                "codex.accountCost.localEstimateHint",
                "只统计经过 Cockpit API 服务的请求，不是 OpenAI 官方账单",
              )}
            >
              <span>{t("codex.accountCost.usedUsd", "已用美元")}</span>
              <strong>{formatCodexEstimatedCostUsd(accountEstimatedCostUsd)}</strong>
            </div>
            <div>
              <span>{t("codex.accountCost.actualPrice", "实际价格")}</span>
              <strong>{formatAccountActualPrice(account)}</strong>
            ...
            <div
              title={t(
                "codex.accountCost.multiplierHint",
                "实际价格（CNY）÷ 已用美元估值（USD）",
              )}
            >
              <span>{t("codex.accountCost.multiplier", "倍率")}</span>
              <strong>
                {formatCodexAccountCostMultiplier(accountCostMultiplier)}
              </strong>
            </div>
          </div>
          {!isApiKeyAccount && (
          ...
          <td>
            <div className="account-cell">
              ...
              <div className="account-sub-line codex-account-meta-inline">
                <span
                  className="codex-login-subline codex-account-price-inline"
                  title={`${t("codex.accountCost.actualPrice", "实际价格")} ${formatAccountActualPrice(account)}`}
                >
                  <span>{t("codex.accountCost.actualPrice", "实际价格")}</span>{" "}
                  <strong>{formatAccountActualPrice(account)}</strong>
                ...
                {batchImportResult ? (
                  ...
                  <>
                    ...
                    <div className="codex-batch-import-tags-row">
                      <label
                        className="codex-batch-import-tags-label"
                        htmlFor="codex-batch-import-actual-spend"
                      >
                        {t(
                          "codex.batchImport.bulkPriceLabel",
                          "导入后统一价格（元）",
                        )}
                      </label>
                      <input
                        id="codex-batch-import-actual-spend"
                        type="text"
                        inputMode="decimal"
                        className="codex-batch-import-tags-input"
                        value={batchImportActualSpendCnyInput}
                        disabled={batchImportBusy}
                        onChange={(event) =>
                          setBatchImportActualSpendCnyInput(event.target.value)
                        }
                        placeholder={t(
                          "codex.batchImport.bulkPricePlaceholder",
                          "可选，例如 35.00；将应用到全部已选账号",
                        )}
                      />
                    </div>
