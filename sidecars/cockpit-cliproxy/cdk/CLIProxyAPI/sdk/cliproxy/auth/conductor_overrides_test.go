...
func TestManager_Execute_Immediate429RetryPolicy(t *testing.T) {
	prev := quotaCooldownDisabled.Load()
	quotaCooldownDisabled.Store(false)
	t.Cleanup(func() { quotaCooldownDisabled.Store(prev) })
	...
	tests := []struct {
		...
		{
			name:         "bare 429 with default cooling retries immediately",
			authID:       "auth-bare-429-exec",
			requestRetry: 2,
			executeError: &Error{HTTPStatus: http.StatusTooManyRequests, Message: "rate limited"},
			wantCalls:    3,
		},
		{
			name:         "retry-after zero with default cooling retries immediately",
			authID:       "auth-zero-retry-after-429-exec",
			requestRetry: 2,
			executeError: &retryAfterStatusError{
				status:     http.StatusTooManyRequests,
		...
		{
			name:         "request-retry zero does not retry",
			authID:       "auth-retry-disabled-429-exec",
			requestRetry: 0,
			executeError: &Error{HTTPStatus: http.StatusTooManyRequests, Message: "rate limited"},
			wantCalls:    1,
		},
		{
			name:           "bare 429 with cooling disabled retries immediately",
			authID:         "auth-bare-429-no-cooling-exec",
			disableCooling: true,
			requestRetry:   2,
			executeError:   &Error{HTTPStatus: http.StatusTooManyRequests, Message: "rate limited"},
			wantCalls:      3,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			m := NewManager(nil, nil, nil)
			// A zero max wait forbids cooldown waits but must not disable an
			// immediate retry for a bare/zero-delay 429.
			m.SetRetryConfig(tc.requestRetry, 0, 0)

			executor := &authFallbackExecutor{
				id: "claude",
				executeErrors: map[string]error{
					tc.authID: tc.executeError,
				},
			}
			m.RegisterExecutor(executor)
			...
			model := "test-model-" + tc.authID
			reg := registry.GetGlobalRegistry()
			reg.RegisterClient(auth.ID, "claude", []*registry.ModelInfo{{ID: model}})
			t.Cleanup(func() { reg.UnregisterClient(auth.ID) })

			_, errExecute := m.Execute(
				context.Background(),
				[]string{"claude"},
			...
			if errExecute == nil || statusCodeFromError(errExecute) != http.StatusTooManyRequests {
				t.Fatalf("execute error = %v, want 429", errExecute)
			}
			if calls := executor.ExecuteCalls(); len(calls) != tc.wantCalls {
				t.Fatalf("execute calls = %d, want %d", len(calls), tc.wantCalls)
			}
			...
			if !state.NextRetryAfter.IsZero() || state.Quota.Exceeded {
				t.Fatalf("bare/zero-delay 429 should not cool down the model, got %#v", state)
			}
			if count := reg.GetModelCount(model); count <= 0 {
				t.Fatalf("bare/zero-delay 429 should not suspend the model, model count = %d", count)
			}
		})
	}
}
