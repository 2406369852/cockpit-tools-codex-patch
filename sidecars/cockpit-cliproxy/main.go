...
func compareAccountSpecs(left, right *accountSpec, strategy string) int {
	...
	case "quota_low_first":
		if cmp := compareQuotaLowFirst(valueInt(left, "quota"), valueInt(right, "quota")); cmp != 0 {
			return cmp
		}
		return compareIntPtrDesc(valueInt(left, "plan"), valueInt(right, "plan"))
	case "plan_low_first":
...
// compareQuotaLowFirst keeps usable low balances ahead of accounts whose
// quota is unknown, while exhausted or malformed snapshots remain last.
func compareQuotaLowFirst(left, right *int) int {
	leftClass := quotaLowFirstClass(left)
	rightClass := quotaLowFirstClass(right)
...
func quotaLowFirstClass(value *int) int {
	switch {
	case value != nil && *value > 0 && *value <= 100:
		return 0
	case value == nil:
		return 1
	default:
...
func main() {
	...
	parentLeaseFile := flag.String("parent-lease-file", "", "Cockpit Tools parent process lease file")
	flag.Parse()
	...
	monitorParentProcessWithLease(ctx, *parentPID, *parentLeaseFile, cancel, emitter)
