...
func TestQuotaLowFirstOrdersPositiveUnknownThenExhaustedOrInvalid(t *testing.T) {
	accounts := []*accountSpec{
		{ID: "zero", AuthID: "zero.json", RemainingQuota: intPtrForTest(0)},
		{ID: "unknown", AuthID: "unknown.json"},
		{ID: "high-positive", AuthID: "high-positive.json", RemainingQuota: intPtrForTest(79)},
		{ID: "negative", AuthID: "negative.json", RemainingQuota: intPtrForTest(-1)},
		{ID: "low-positive", AuthID: "low-positive.json", RemainingQuota: intPtrForTest(2)},
		{ID: "over-range", AuthID: "over-range.json", RemainingQuota: intPtrForTest(101)},
	}
	m := &manifest{
		RoutingStrategy:   "quota_low_first",
		Accounts:          make([]accountSpec, 0, len(accounts)),
		accountByAuthID:   make(map[string]*accountSpec, len(accounts)),
		accountByID:       make(map[string]*accountSpec, len(accounts)),
		originalIndexByID: make(map[string]int, len(accounts)),
	}
	auths := make([]*coreauth.Auth, 0, len(accounts))
	for index, account := range accounts {
		m.Accounts = append(m.Accounts, *account)
		m.accountByAuthID[account.AuthID] = account
		m.accountByID[account.ID] = account
		m.originalIndexByID[account.ID] = index
		auths = append(auths, &coreauth.Auth{ID: account.AuthID})
	}

	ordered := (&cockpitSelector{manifest: m}).orderAuths(auths, 0)
	...
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("quota_low_first order = %#v, want %#v", got, want)
	}
}
