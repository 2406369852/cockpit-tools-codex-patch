...
> **Edição comunitária v1.3.23-patch.1 (Windows x64)**: inclui roteamento da API local do Codex, preços/custos persistentes, retentativa automática para erro 429 e atualização segura do aplicativo. Baixe o instalador na [Release do repositório desta edição](https://github.com/2406369852/cockpit-tools-codex-patch/releases/tag/v1.3.23-patch.1). O arquivo `.exe` é um instalador, não um arquivo compactado; clique duas vezes para instalar. A compilação não é assinada, então o SmartScreen pode exibir um aviso.
